...
namespace YuGiOhV2.Core
{
    public class Events
    {
        ...
        private async Task HandleCommand(SocketMessage message)
        {
            ...
            if ((possibleCmd.HasStringPrefix(prefix, ref argPos) ||
                ...
                && possibleCmd.Content.Trim() != prefix)
            {
                ...
                if (message.Channel is SocketDMChannel)
                    ...
                    AltConsole.Print("Info", "Command",
                        $"{possibleCmd.Author.Username} from {(possibleCmd.Channel as SocketTextChannel)?.Guild.Name}");
                ...
                if (!result.IsSuccess)
                {
                    ...
                    if (result.ErrorReason.ToLower().Contains("you are currently in timeout"))
                        await context.Channel.SendMessageAsync(
                            "Please wait 5 seconds between each type of paginator command!");
        ...
        private void RegisterLogging()
        {
            _client.Log += message
                => Task.Run(()
                    => AltConsole.Print(message.Severity.ToString(), message.Source, message.Message,
                        message.Exception));
