...
namespace YuGiOhV2.Services
{
    public class Web
    {
        ...
        public async Task<HttpResponseMessage> Post(string url, string content, string authorization = null,
        ...
        public async Task<YuGiOhPrices> GetPrices(string name, string realName = null)
        {
            ...
            if ((response == null || response.Status == "fail") && !string.IsNullOrEmpty(realName))
                response = await GetDeserializedContent<YuGiOhPrices>(
                    $"{PricesBaseUrl}{Uri.EscapeUriString(realName)}");
